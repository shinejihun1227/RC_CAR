...
| [v4](../v4/README.md) | 배열 인덱스, volatile, ISR, 임계 구역, 시간 차, 누적값 | `pulses[]`, `updatePid()`, P/I/D 계산 | 좌우 구동륜 1회전 펄스를 실측해 PPR 입력, Kp부터 조정 | 좌우 RPM 기록, 목표값과 오차·진동 비교 |
...
초반에는 `pinMode()`를 핀마다 작성해 입출력을 익힌다. v2에서는 MAC 주소와 패킷으로 배열·구조체를 배우고, v4에서 좌우 두 구동축을 배열 인덱스로 처리한다. 뒤 수동 바퀴에는 제어 신호나 엔코더를 연결하지 않는다.
