...
기본 모터 주행부터 거리 감지, 무선 조종, 손 제스처, 2구동륜 PID 속도 제어까지 직접 구현하는 강의용 저장소입니다. **ESP32에 Arduino IDE로 코드를 업로드하고, C언어 문법 중심으로 실습합니다.** 수업에 필요한 코드·배선·부품·설계를 모았습니다.
...
| [v4 2구동륜 속도 제어](v4/README.md) | 좌우 구동륜 RPM을 보정하고 뒤 수동 바퀴가 따라오는 차량 | 인터럽트, 펄스·RPM, 배열, 시간 간격, PID | 엔코더 N20 2개, 수동 바퀴 2개, TB6612FNG 1개 |

v0~v3은 TT 모터 2개로 기능을 익힙니다. v4에서는 좌우 N20 엔코더 모터 2개와 뒤쪽 수동 바퀴 2개로 구동계를 바꾸며 차량 핀맵도 바뀝니다. v4는 v2 조이스틱 조종기 또는 v3 제스처 조종기를 사용할 수 있습니다.
...
차량 전원은 1.5V AA 알카라인 4개(공칭 6V)와 ESP32용 5V 벅-부스트를 기준으로 합니다. 조종기 전원은 1셀 배터리를 보호·충전 회로와 3.3V 정전압 레귤레이터를 거쳐 공급합니다. 모터 전원과 로직 전원을 구분하고 각 장치 안에서 GND를 공통으로 연결합니다. 처음 모터를 시험할 때는 바퀴를 바닥에서 띄웁니다.
