...
이 폴더는 좌우 N20 엔코더 구동과 뒤쪽 수동 바퀴 2개를 사용하는 RC카 샤시를 설계할 때 부품을 Fusion 360 화면에 함께 띄워 놓기 위한 **배치·간섭 확인용 참조 모델**입니다.

실측 치수로 장착부를 만들려면 [실측 부품 기반 설계 안내](../fusion360/MEASURED_PARTS_WORKFLOW.md)를 먼저 확인하세요. 아래 STL은 실측 CAD가 아니며, 자동 생성기는 모든 치수·배치·축 관계가 연동되는 조립체가 아닙니다.
...
- TB6612FNG 모터 드라이버 × 1
- 4칸 AA 배터리 홀더 × 1
- 5V 2A 이상 벅-부스트 컨버터 × 1 (차량 ESP32용)
- VL53L1X ToF 거리 센서 × 1
- M3 스페이서·나사·인서트
- 전원 스위치, 커패시터, 배선과 커넥터
